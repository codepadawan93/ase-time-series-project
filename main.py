...
plt.plot(residual, label='Residuals (noise)', color='green')
plt.legend(loc='best')
plt.show()
test_stationarity(residual, DAYS)
plt.show()

# Plot ACF and PACF to find p, d, q
lag_acf = acf(ts_log_diff, nlags=20, missing='conservative')
...
predictions2 = results_ARIMA.forecast(1000)
df.add(np.exp(predictions2[0]))
plt.plot(df)
plt.show()
